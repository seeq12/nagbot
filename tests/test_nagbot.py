...
class TestNewNagbot(unittest.TestCase):
    ...
    def test_stoppable(self):
        ...
        assert sqaws.is_stoppable(past_date, ec2_type, todays_date) is True
        assert sqaws.is_stoppable(today_date, ec2_type, todays_date) is True
        assert sqaws.is_stoppable(on_weekends, ec2_type, todays_date, is_weekend=True) is True
        assert sqaws.is_stoppable(unknown_date, ec2_type, todays_date) is True
        assert sqaws.is_stoppable(past_date_warned, ec2_type, todays_date) is True
        assert sqaws.is_stoppable(today_date_warned, ec2_type, todays_date) is True
        assert sqaws.is_stoppable(anything_warned, ec2_type, todays_date) is True
        assert sqaws.is_stoppable(on_weekends_warned, ec2_type, todays_date, is_weekend=True) is True

        # These instances should NOT get a stop warning
        assert sqaws.is_stoppable(on_weekends, ec2_type, todays_date, is_weekend=False) is False
        assert sqaws.is_stoppable(on_weekends_warned, ec2_type, todays_date, is_weekend=False) is False
        assert sqaws.is_stoppable(wrong_state, ec2_type, todays_date) is False
        assert sqaws.is_stoppable(future_date, ec2_type, todays_date) is False

        # These instances don't have a warning, so they shouldn't be stopped yet
        assert sqaws.is_safe_to_stop(past_date, ec2_type, todays_date) is False
        assert sqaws.is_safe_to_stop(today_date, ec2_type, todays_date) is False
        assert sqaws.is_safe_to_stop(on_weekends, ec2_type, todays_date, is_weekend=True) is False
        assert sqaws.is_safe_to_stop(unknown_date, ec2_type, todays_date) is False
        assert sqaws.is_safe_to_stop(wrong_state, ec2_type, todays_date) is False
        assert sqaws.is_safe_to_stop(future_date, ec2_type, todays_date) is False

        # These instances can be stopped right away
        assert sqaws.is_safe_to_stop(past_date_warned, ec2_type, todays_date) is True
        assert sqaws.is_safe_to_stop(today_date_warned, ec2_type, todays_date) is True
        assert sqaws.is_safe_to_stop(on_weekends_warned, ec2_type, todays_date, is_weekend=True) is True
        assert sqaws.is_safe_to_stop(anything_warned, ec2_type, todays_date) is True
    ...
    def test_terminatable(self):
        ...
        assert sqaws.Instance.is_terminatable(past_date, todays_date) is True
        assert sqaws.Instance.is_terminatable(today_date, todays_date) is True
        assert sqaws.Instance.is_terminatable(past_date_warned, todays_date) is True
        assert sqaws.Instance.is_terminatable(today_date_warned, todays_date) is True

        # These instances should NOT get a termination warning
        assert sqaws.Instance.is_terminatable(wrong_state, todays_date) is False
        assert sqaws.Instance.is_terminatable(future_date, todays_date) is False
        assert sqaws.Instance.is_terminatable(unknown_date, todays_date) is False
        assert sqaws.Instance.is_terminatable(anything_warned, todays_date) is False

        # These instances don't have a warning, so they shouldn't be terminated yet
        assert sqaws.Instance.is_safe_to_terminate(past_date, todays_date) is False
        assert sqaws.Instance.is_safe_to_terminate(today_date, todays_date) is False
        assert sqaws.Instance.is_safe_to_terminate(unknown_date, todays_date) is False
        assert sqaws.Instance.is_safe_to_terminate(wrong_state, todays_date) is False
        assert sqaws.Instance.is_safe_to_terminate(future_date, todays_date) is False
        assert sqaws.Instance.is_safe_to_terminate(anything_warned, todays_date) is False

        # These instances can be terminated, but not yet
        assert sqaws.Instance.is_safe_to_terminate(past_date_warned, todays_date) is False
        assert sqaws.Instance.is_safe_to_terminate(today_date_warned, todays_date) is False

        # These instances have a warning, but are not eligible to add a warning, so we don't terminate
        assert sqaws.Instance.is_safe_to_terminate(anything_warned_days_ago, todays_date) is False

        # These instances can be terminated now
        assert sqaws.Instance.is_safe_to_terminate(past_date_warned_days_ago, todays_date) is True
    ...
    def test_deletable(self):
        ...
        assert sqaws.Volume.is_terminatable(past_date, todays_date) is True
        assert sqaws.Volume.is_terminatable(today_date, todays_date) is True
        assert sqaws.Volume.is_terminatable(past_date_warned, todays_date) is True
        assert sqaws.Volume.is_terminatable(today_date_warned, todays_date) is True

        # These volumes should NOT get a deletion warning
        assert sqaws.Volume.is_terminatable(wrong_state, todays_date) is False
        assert sqaws.Volume.is_terminatable(future_date, todays_date) is False
        assert sqaws.Volume.is_terminatable(unknown_date, todays_date) is False
        assert sqaws.Volume.is_terminatable(anything_warned, todays_date) is False

        # These volumes don't have a warning, so they shouldn't be deleted yet
        assert sqaws.Volume.is_safe_to_terminate(past_date, todays_date) is False
        assert sqaws.Volume.is_safe_to_terminate(today_date, todays_date) is False
        assert sqaws.Volume.is_safe_to_terminate(unknown_date, todays_date) is False
        assert sqaws.Volume.is_safe_to_terminate(wrong_state, todays_date) is False
        assert sqaws.Volume.is_safe_to_terminate(future_date, todays_date) is False
        assert sqaws.Volume.is_safe_to_terminate(anything_warned, todays_date) is False

        # These volumes can be deleted, but not yet
        assert sqaws.Volume.is_safe_to_terminate(past_date_warned, todays_date) is False
        assert sqaws.Volume.is_safe_to_terminate(today_date_warned, todays_date) is False

        # These volumes have a warning, but are not eligible to add a warning, so we don't delete
        assert sqaws.Volume.is_safe_to_terminate(anything_warned_days_ago, todays_date) is False

        # These volumes can be deleted now
        assert sqaws.Volume.is_safe_to_terminate(past_date_warned_days_ago, todays_date) is True
