...
#
#        assert running_no_stop_after.generic_is_stoppable_without_warning() is True
#        assert stopped_no_stop_after.generic_is_stoppable_without_warning() is False
#        assert past_date.generic_is_stoppable_without_warning() is False
#        assert notified.generic_is_stoppable_without_warning() is True
#        # During weekdays do not delete instance with stop after tag "On Weekend"
#        assert weekend.generic_is_stoppable_without_warning(is_weekend=False) is False
#        # On Weekend delete instance with stop after tag "On Weekend"
#        assert weekend.generic_is_stoppable_without_warning(is_weekend=True) is True
#
...
#        anything_warned = self.setup_instance(state='stopped', terminate_after='Yummy Udon Noodles' +
        #        today_warning_str)
#
#        old_warning_str = ' (Nagbot: Warned on ' + resource.MIN_TERMINATION_WARNING_YYYY_MM_DD + ')'
#        past_date_warned_days_ago = self.setup_instance(state='stopped', terminate_after='2019-01-01' +
        #        old_warning_str)
...
#
    @staticmethod
    @patch('app.instance.boto3.client')
    def test_is_stoppable_without_warning(mock_client):
        mock_instance = TestInstance.setup_instance(state='running')
        assert mock_instance.is_stoppable_without_warning() is True
